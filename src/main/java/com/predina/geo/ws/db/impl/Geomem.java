...
public class Geomem<T, R> {
     ...
     * {@link Info}.
     */
    private final Map<Long, SortedMap<Long, Info<T, R>>> mapByGeoHash = new ConcurrentHashMap<>(2315754, 0.75f);

    /**
     * Records a mapByGeoHash as above for each id of type R.
     */
    private final Map<R, Map<Long, SortedMap<Long, Info<T, R>>>> mapById = new ConcurrentHashMap<>(2315754, 0.75f);
    ...
    public void add(Info<T, R> info) {
        String hash = GeoHash.encodeHash(info.lat(), info.lon(), 28);
